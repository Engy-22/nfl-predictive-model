# Calculates quarterback efficiency stats by week.
...
air_yards <-
  ...
  filter(pass == TRUE,
         ...
         !is.na(air_yards),
         !is.na(air_epa)) %>%
  group_by(quarterback = passer_player_name, season, week) %>%
  summarize(
...
scramble_epa <-
  ...
  group_by(quarterback = rusher_player_name, season, week) %>%
  summarize(
    scrambles = n(),
...
consolidated_epa <-
  ...
  select(quarterback, season, week, dropbacks, dropback_epa, 
         passes, passing_epa, scrambles, scramble_epa) %>%
  arrange(quarterback, season)

qbs_historical_by_week <-
  left_join(consolidated_epa, air_yards,
            by = c("quarterback", "season", "week"))

write_rds(qbs_historical_by_week, "data/quarterbacks_historical_byweek.rds")
